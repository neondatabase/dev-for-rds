...
const ActionJob = ({ id, text, time, position = 'middle' }) => {
  return (
    <div className='relative flex gap-2 items-center bg-brand-surface border border-brand-border rounded p-3 text-white text-xs min-w-[220px] min-h-10'>
      <ActionSpinner id={text} className='absolute w-4 h-4' />
      <ActionCheck id={text} className='absolute w-4 h-4 scale-0' />
      <div className='flex justify-between mb-0.5 ml-6 mr-1 w-full'>
        <span>{text}</span>
